...
// Načítanie všetkých otázok a odpovedí z databázy
$otazky_a_odpovede = $qna->getQnA();
?>
...
<body>
  ...
  <main>
    ...
    <section class="container">
      <!-- Dynamicky zobrazené otázky a odpovede z databázy -->
      <?php 
      if (!empty($otazky_a_odpovede)) {
          foreach ($otazky_a_odpovede as $item): ?>
            <div class="accordion">
              <div class="question"><?php echo htmlspecialchars($item['otazka']); ?></div>
              <div class="answer"><?php echo nl2br(htmlspecialchars($item['odpoved'])); ?></div>
            </div>
          <?php endforeach; 
      } else {
          echo "<p>Žiadne otázky a odpovede nie sú k dispozícii.</p>";
      }
      ?>
    </section>
  </main>
